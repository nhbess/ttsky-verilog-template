...
"""
Experiment 17 — error-driven local mutation (Exp16 objective + error-biased gate pick).

Same three arms as experiment 16 for evaluation, but variants 1–2 sample the mutable gate as
P(u) ∝ (e_u + eps)^alpha after clamp+relax on a random row (instead of uniform u).

Run from repo root:
  python src/experiments/experiment_17.py

Outputs:
  src/results/experiment_17_errpick_alignment.png
  src/results/experiment_17_curve_<scenario_idx>.png
  src/results/experiment_17_Elocal_<scenario_idx>.png
  src/results/experiment_17_depth_<scenario_idx>.png
"""
...
import experiment_17_worker as e17w  # noqa: E402

RESULTS = SRC_ROOT / "results"
OUT_FIG = RESULTS / "experiment_17_errpick_alignment.png"
...
VAR_SHORT = ["supervised", "err-pick (LUT)", "err-pick + DAG"]
...
def collect_scenario(
    ...
    with Pool(processes=workers) as pool:
        rows: List[Dict[str, Any]] = pool.map(e17w.run_one, jobs)
...
def _default_workers() -> int:
    for key in ("EXPERIMENT_17_WORKERS", "EXPERIMENT_16_WORKERS", "EXPERIMENT_14_WORKERS"):
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(description="Experiment 17: error-biased gate selection + clamp local E")
    ...
    fig.suptitle(
        rf"Experiment 17 — $P(u)\propto (e_u+\epsilon)^{{\alpha}}$; same clamp $E_{{\mathrm{{local}}}}$ as Exp16 ($K={args.k}$)",
        fontsize=10,
    )
    ...
    for si, (slab, n_in, n_out, kind, ts_fix, max_ticks, n_seeds) in enumerate(SCENARIOS):
        ...
        print(
            f"  success: sup={d['sr'][0]:.2f}  ep={d['sr'][1]:.2f}  ep+DAG={d['sr'][2]:.2f}  "
        ...
        out_c = RESULTS / f"experiment_17_curve_{si}.png"
        ...
        axe.set_ylabel(r"mean $E_{\mathrm{local}}$ (last baseline sample)")
        axe.set_title(f"{slab} — internal propagated error vs time (variants 1–2)")
        axe.legend(fontsize=7)
        axe.grid(True, alpha=0.3)
        out_e = RESULTS / f"experiment_17_Elocal_{si}.png"
        ...
        out_d = RESULTS / f"experiment_17_depth_{si}.png"
