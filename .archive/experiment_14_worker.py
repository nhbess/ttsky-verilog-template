"""
Picklable worker for experiment 14 (supervised vs Exp13 local vs local predictive DAG).

Job:
  (variant, n_in, n_out, kind, traj_seed, max_ticks, fixed_table_seed, extra_gates, log_stride)

variant 0 = ``Exp12AdaptiveDAG`` (global score COMPARE)
variant 1 = ``TTAdaptiveDAGLocalUnsupervised`` (Exp13 stats / corr)
variant 2 = ``TTAdaptiveDAGLocalPredictive`` (Exp14 LOO prediction error vs child)
"""
...
def run_one(
    ...
    if variant == 0:
        ...
        m = adlp.TTAdaptiveDAGLocalPredictive(
            n_in=n_in,
            n_out=n_out,
            G=G,
            target=tgt,
