...
"""Experiment 8 policy variants (multi-gate ``TTChainLearnerExp72Multi`` + same hooks as exp 7.2)."""
...
class PlasticMappedPlateauChainMulti(e72m.TTChainLearnerExp72Multi):
...
class RecentSuccessExploitPlateauChainMulti(e72m.TTChainLearnerExp72Multi):
    ...
    def _after_compare_hook(self) -> None:
        ...
        if self._gates_mutated_last_tick and self.new_score > self.old_score:
            units = (
                self._multi_units
                if self.mutation_batch > 1 and self._multi_units
                else [self.unit_sel]
            )
            for u in units:
...
class MixedPlasticSuccessPlateauChainMulti(e72m.TTChainLearnerExp72Multi):
    _PLASTIC_MASK: Tuple[int, int, int, int] = (15, 7, 3, 1)
