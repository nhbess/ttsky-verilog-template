...
"""
Experiment 18 — local LUT statistics from clamp-relax (local model fitting).

Variants 1–2: fixed DAG, no rewiring. After clamp+relax, each gate uses a **counterfactual**
hidden target: pin u to 0/1, re-forward downstream, pick b that matches clamped y* at the
subnet output (output gate uses y* directly). EMA over a simplex p_u(g) on the 16 gate types
(reinforcing all g with f_g(x_a,x_b)=y_u^*), then one random gate refits by argmax (variant 1)
or weighted sample (variant 2).

Variant 0: supervised Exp12 baseline. External score only for evaluation.

Run from repo root:
  python src/experiments/experiment_18.py

Outputs:
  src/results/experiment_18_local_lut_alignment.png
  src/results/experiment_18_curve_<scenario_idx>.png
  src/results/experiment_18_Elocal_<scenario_idx>.png
  src/results/experiment_18_depth_<scenario_idx>.png
"""
...
import experiment_18_worker as e18w  # noqa: E402

RESULTS = SRC_ROOT / "results"
OUT_FIG = RESULTS / "experiment_18_local_lut_alignment.png"
...
VAR_SHORT = ["supervised", "local fit (argmax)", "local fit (softmax)"]
...
def collect_scenario(
    ...
    with Pool(processes=workers) as pool:
        rows: List[Dict[str, Any]] = pool.map(e18w.run_one, jobs)
...
def _default_workers() -> int:
    for key in ("EXPERIMENT_18_WORKERS", "EXPERIMENT_17_WORKERS", "EXPERIMENT_16_WORKERS"):
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(description="Experiment 18: local LUT stats from clamp-relax")
    ...
    fig.suptitle(
        rf"Experiment 18 — EMA $p_u(g)$ over gate types; refit one gate/tick ($K={args.k}$)",
        fontsize=10,
    )
    ...
    for si, (slab, n_in, n_out, kind, ts_fix, max_ticks, n_seeds) in enumerate(SCENARIOS):
        ...
        ax2.set_title(f"{slab} — propagated error after last sample")
        ax2.grid(True, axis="y", alpha=0.3)

        print(
            f"  success: sup={d['sr'][0]:.2f}  argmax={d['sr'][1]:.2f}  sm={d['sr'][2]:.2f}  "
        ...
        out_c = RESULTS / f"experiment_18_curve_{si}.png"
        ...
        axe.set_xlabel("training tick")
        axe.set_ylabel(r"mean $E_{\mathrm{local}}$")
        axe.set_title(f"{slab} — internal error vs time (variants 1–2)")
        axe.legend(fontsize=7)
        axe.grid(True, alpha=0.3)
        out_e = RESULTS / f"experiment_18_Elocal_{si}.png"
        ...
        axd.set_xlabel("training tick")
        axd.set_ylabel("mean backward depth (gates with $e>0$)")
        axd.set_title(f"{slab} — error depth vs time (variants 1–2)")
        axd.legend(fontsize=7)
        axd.grid(True, alpha=0.3)
        out_d = RESULTS / f"experiment_18_depth_{si}.png"
