...
"""
Experiment 15 — Exp14 tasks/evaluation + **probabilistic gate adaptation**.

Each gate keeps 16 logits (softmax → p over LUT nibbles). On each LUT trial, sample
``i ~ p``; on accept ``w_i += η``, on reject ``w_i -= λ``. Logits are **not** reset when
the stored LUT changes (memory). Default: η=1, λ=0.

Same three arms as Exp14 (supervised / local unsup / local predictive).

Run from repo root:
  python src/experiments/experiment_15.py

Outputs:
  src/results/experiment_15_softmax_alignment.png
  src/results/experiment_15_curve_<scenario_idx>.png
  src/results/experiment_15_op_hist_<scenario_idx>.png
"""
...
import experiment_15_worker as e15w  # noqa: E402

RESULTS = SRC_ROOT / "results"
OUT_FIG = RESULTS / "experiment_15_softmax_alignment.png"
...
def collect_scenario(
    ...
    with Pool(processes=workers) as pool:
        rows: List[Dict[str, Any]] = pool.map(e15w.run_one, jobs)
    ...
    ok_ct = [0, 0, 0]
    n_per = [0, 0, 0]
    op_hist_sum = np.zeros((3, 16), dtype=float)
    for r in rows:
        vi = int(r["variant"])
        n_per[vi] += 1
        if r["ok_task"]:
        ...
        mean_ent[vi] += float(r["mean_gate_entropy_bits"])
        op_hist_sum[vi] += np.asarray(r["op_hist"], dtype=float)
    for vi in range(3):
        sr[vi] = ok_ct[vi] / max(1, n_per[vi])
        fin[vi] = fin[vi] / max(1, n_per[vi])
        mean_ent[vi] = mean_ent[vi] / max(1, n_per[vi])
        op_hist_sum[vi] /= max(1, n_per[vi])
    fin_mean = [float(fin[0]), float(fin[1]), float(fin[2])]
    mean_ent_list = [float(mean_ent[0]), float(mean_ent[1]), float(mean_ent[2])]

    return {
        "sr": sr,
        "fin_mean": fin_mean,
        "mean_gate_entropy_bits": mean_ent_list,
        "op_hist_mean": op_hist_sum,
        "rows": rows,
...
def _default_workers() -> int:
    for key in ("EXPERIMENT_15_WORKERS", "EXPERIMENT_14_WORKERS", "EXPERIMENT_12_WORKERS"):
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(description="Experiment 15: softmax gate adaptation vs Exp14 arms")
    ...
    fig.suptitle(
        rf"Experiment 15 — softmax proposals ($K={args.k}$); mean final $H(p)$ per gate (bits)",
        fontsize=10,
    )
    ...
    for si, (slab, n_in, n_out, kind, ts_fix, max_ticks, n_seeds) in enumerate(SCENARIOS):
        ...
        ax2.bar(xb, d["mean_gate_entropy_bits"], w, color=colors, edgecolor="black", linewidth=0.35)
        ax2.set_xticks(xb)
        ax2.set_xticklabels(VAR_SHORT, fontsize=7, rotation=14, ha="right")
        ax2.set_ylabel("mean H(p) per gate (bits)")
        ax2.set_ylim(0, 4.05)
        ax2.axhline(4.0, color="gray", ls="--", lw=0.7, alpha=0.55, label="uniform (4 bits)")
        ax2.set_title(f"{slab} — distribution sharpness (final)")
        ...
        print(
            ...
            f"mean H: {d['mean_gate_entropy_bits'][0]:.3f} / {d['mean_gate_entropy_bits'][1]:.3f} / "
            f"{d['mean_gate_entropy_bits'][2]:.3f}"
        )
        ...
        out_c = RESULTS / f"experiment_15_curve_{si}.png"
        RESULTS.mkdir(parents=True, exist_ok=True)
        fig2.savefig(out_c, dpi=150)
        plt.close(fig2)
        print(f"  wrote {out_c}")

        fig3, axh = plt.subplots(figsize=(9, 3.4), constrained_layout=True)
        xh = np.arange(16)
        bw = 0.25
        for vi, c in enumerate(colors):
            axh.bar(xh + (vi - 1) * bw, d["op_hist_mean"][vi], bw, color=c, label=VAR_SHORT[vi], edgecolor="black", lw=0.2)
        axh.set_xlabel("sampled LUT nibble")
        axh.set_ylabel("mean count per seed")
        axh.set_title(f"{slab} — sampled operator histogram (LUT trials only)")
        axh.legend(fontsize=7)
        axh.set_xticks(xh)
        axh.grid(True, axis="y", alpha=0.3)
        out_h = RESULTS / f"experiment_15_op_hist_{si}.png"
        fig3.savefig(out_h, dpi=150)
        plt.close(fig3)
        print(f"  wrote {out_h}")
