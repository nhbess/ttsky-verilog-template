...
"""
Experiment 2 — 3-bit parity (a^b^c), topology h1=f(a,b), h2=f(b,c), y=f(h1,h2).

Independent from experiment_1 (XOR). Uses ref/tt_parity3_spec.py only.

Plateau mode uses ~1/4 tie acceptance (plateau_mask=3); strict is new>old only.

RTL check (separate sim, does not touch XOR build): python run_sim_parity3.py

Outputs (src/results/): experiment_2_strict.png, experiment_2_plateau.png, experiment_2_compare.png

Run from repo root: python src/experiments/experiment_2.py
"""
...
TRAJECTORY_MAX_STEPS = 1200
OVERLAY_SEEDS = TRAJECTORY_SEEDS[:4]
BATCH_SEED_START = 1
# Parity is harder than XOR — raise caps if you need tighter statistics (runtime grows ~linearly).
BATCH_SEED_COUNT = 48
BATCH_MAX_TICKS = 30_000
MAX_SCORE = 8

RESULTS = SRC_ROOT / "results"
OUT_STRICT = RESULTS / "experiment_2_strict.png"
OUT_PLATEAU = RESULTS / "experiment_2_plateau.png"
OUT_COMPARE = RESULTS / "experiment_2_compare.png"
...
def trajectory(
    ...
    m = spec.TTParity3Learner(plateau_escape=plateau_escape)
    m.reset(seed=seed)
    scores: list[int] = [m.score_current_gates()]
    fsm_ids: list[int] = [int(m.fsm)]
    converged = scores[0] == MAX_SCORE
...
def batch_run(
    ...
    for s in range(seed_start, seed_start + count):
        ...
        ok, ticks = m.run_until_parity(max_ticks=max_ticks)
...
def _single_figure(
    ...
    rule = "plateau (sidewalk)" if plateau_escape else "strict (new > old only)"
    fig.suptitle(
        f"Experiment 2 — 3-bit parity — {tag}\n"
        f"{rule}; {BATCH_SEED_COUNT} seeds × {BATCH_MAX_TICKS} ticks; score 0–{MAX_SCORE}",
        fontsize=11,
    )
    ...
    for seed in TRAJECTORY_SEEDS:
        ...
        label = f"0x{seed:04X}" + (" ✓" if conv and scores[-1] == MAX_SCORE else "")
        ax0.plot(x, scores, alpha=0.85, linewidth=1.1, label=label)
    ax0.set_xlabel("Training tick")
    ax0.set_ylabel(f"Global parity score (0–{MAX_SCORE})")
    ax0.set_ylim(-0.2, MAX_SCORE + 0.3)
    ax0.set_title("Score vs time")
    ax0.legend(loc="lower right", fontsize=7, ncol=2)
    ax0.grid(True, alpha=0.3)

    ax1 = axes[0, 1]
    demo_seed = TRAJECTORY_SEEDS[0]
    _, fsm_ids, _ = trajectory(demo_seed, min(TRAJECTORY_MAX_STEPS, 600), plateau_escape)
    ax1.plot(range(len(fsm_ids)), fsm_ids, color="C0", linewidth=0.8)
    ax1.set_xlabel("Training tick")
    ax1.set_ylabel("FSM state id")
    ax1.set_title(f"FSM trace (seed 0x{demo_seed:04X})")
    ...
    if success_ticks:
        ax2.hist(
            success_ticks,
            bins=min(30, max(10, len(success_ticks) // 3)),
            color="steelblue",
            edgecolor="white",
        )
        ax2.axvline(
        ...
        ax2.legend()
    ax2.set_xlabel("Ticks until perfect parity (score 8)")
    ax2.set_ylabel("Count")
    ax2.set_title(f"Convergence time (seeds {BATCH_SEED_START}..)")
    ...
    lines = [
        f"Batch: {BATCH_SEED_COUNT} seeds, max {BATCH_MAX_TICKS} ticks",
        f"Success: {ok_n} ({100.0 * rate:.1f}%)",
        f"Failed: {fail_n}",
    ]
    if success_ticks:
        lines.append(f"Ticks — min: {min(success_ticks)}, max: {max(success_ticks)}")
        lines.append(f"Median: {float(np.median(success_ticks)):.0f}")
    ax3.text(0.02, 0.72, "\n".join(lines), transform=ax3.transAxes, va="top", fontsize=9, family="monospace")

    ax3_bar = ax3.inset_axes([0.02, 0.38, 0.92, 0.12])
    ax3_bar.barh(0, ok_n, height=0.5, color="seagreen", label="ok")
    ax3_bar.barh(0, fail_n, left=ok_n, height=0.5, color="indianred", label="fail")
    ...
    ax3_strip.set_title(f"First {strip_n} seeds", fontsize=8)
...
def _compare_figure(st_strict: BatchStats, st_plateau: BatchStats, np, plt) -> None:
    fig, axes = plt.subplots(2, 2, figsize=(12, 9), constrained_layout=True)
    fig.suptitle(
        f"Experiment 2 — strict vs plateau (3-bit parity)\n"
        f"{BATCH_SEED_COUNT} seeds × {BATCH_MAX_TICKS} ticks",
        fontsize=11,
    )
    ...
    hi = min(int(r_max * 1.05) + 1, BATCH_MAX_TICKS)
    ...
    axh.set_xlabel("Ticks until score 8")
    ...
    for i, seed in enumerate(OVERLAY_SEEDS):
        ...
        c = colors[i]
        axt.plot(s_st, color=c, linestyle="--", linewidth=1.0, alpha=0.9)
        axt.plot(s_pl, color=c, linestyle="-", linewidth=1.0, alpha=0.9, label=f"0x{seed:04X}")
    axt.set_xlabel("Training tick")
    axt.set_ylabel("Score")
    axt.set_ylim(-0.2, MAX_SCORE + 0.3)
    axt.set_title("Solid=plateau, dashed=strict")
    ...
    tbl = [
        ["", "strict", "plateau"],
        ["success", f"{st_strict.ok_n}/{BATCH_SEED_COUNT}", f"{st_plateau.ok_n}/{BATCH_SEED_COUNT}"],
        ["ticks (ok)", fmt_ticks(st_strict.success_ticks, np), fmt_ticks(st_plateau.success_ticks, np)],
    ]
    table = axtbl.table(cellText=tbl, loc="center", cellLoc="center", colWidths=[0.22, 0.39, 0.39])
    table.scale(1.2, 2.0)
...
def main() -> int:
    ...
    st_plateau = _single_figure(True, "plateau", OUT_PLATEAU, np, plt)
